...
import { LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer, TooltipProps, ReferenceLine, Label } from "recharts"
...
const MultiLineChart: React.FC<MultiLineChartProps> = ({ data, movingAverageData, title, param_id, start_date, end_date, selected_zones, zonesIdAndName }) => {
      ...
      <div className="pr-4 bg-white">
        <ResponsiveContainer width="100%" height={300}>
          <LineChart margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
            ...
            <XAxis dataKey="timestamp" name="Fecha" type="number" stroke="#333" allowDuplicatedCategory={false} domain={["dataMin", "dataMax"]} tickFormatter={FormatXAxis} tick={{ fontSize: 12 }} />
            ...
            {filteredAnnotations.map((annotation, index) => (
              <React.Fragment key={index}>
                <ReferenceLine 
                  isFront
                  x={annotation.start_date?.toString()}
                  stroke="#575757"
                  strokeDasharray="3 3"
                ...
                <ReferenceLine 
                  isFront
                  x={annotation.end_date?.toString()}
                  stroke="#575757"
                  strokeDasharray="3 3"
