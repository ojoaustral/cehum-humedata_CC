/*
   ...
   Version: 1.3 modified by Ian Zamora
   Used in Humedatas Toto 1 and Toto 2, because the Humedata Atlas uses a different configuration and PCB.
*/
...
void loop() {
  ...
  delay(1000);
  sleep_sensors(rtd_address);
  sleep_sensors(ph_address);
  sleep_sensors(orp_address);
  ...
  _data_lorawan[26] = uint8_t  ((_data[28] -9) * 255/6);            // Battery voltage
  ...
  _data_lorawan[40] = do_15_float_bytes[0];                            // DO 15 *
  _data_lorawan[41] = do_15_float_bytes[1];                            // DO 15 *
  _data_lorawan[42] = do_15_float_bytes[2];                            // DO 15 *
  _data_lorawan[43] = do_15_float_bytes[3];                            // DO 15 *
  ...
  //delay(360*1000);
  int sleep_times = sleep_time*60;
  LowPower.sleep(sleep_times*1000); // e.g. 10 minutos * 60*segundos * 1000 milisegundos
  digitalWrite(rtd_off_pin, HIGH);
  digitalWrite(ph_off_pin, HIGH);
  digitalWrite(orp_off_pin, HIGH);
  digitalWrite(ec_off_pin, HIGH);
